...
inline void loadTranslation(const QString& tname, const QString& dir = lmms::ConfigManager::inst()->localeDir()) {
...
int main(int argc, char** argv) {
...
#endif
  QCoreApplication* app = coreOnly ? new QCoreApplication(argc, argv) : new gui::MainApplication(argc, argv);
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--version" || arg == "-v") {
      ...
      if (argc > i + 1) {
        // Project bundle file name given
        printf("Making bundle\n");
        dataFile.writeFile(QString::fromLocal8Bit(argv[i + 1]), true);
        return EXIT_SUCCESS;
      } else { return usageError("No project bundle name given"); }
    } else if (arg == "--allowroot") {
      ...
      return EXIT_SUCCESS;
    } else if (arg == "render" || arg == "--render" || arg == "-r" || arg == "rendertracks" || arg == "--rendertracks") {
      ++i;
      ...
      else if (ext == "ogg") { eff = ProjectRenderer::OggFile; }
#endif
#ifdef LMMS_HAVE_MP3LAME
      else if (ext == "mp3") { eff = ProjectRenderer::MP3File; }
#endif
      else if (ext == "flac") { eff = ProjectRenderer::FlacFile; }
      else { return usageError(QString("Invalid output format %1").arg(argv[i])); }
      ...
      else { return usageError(QString("Invalid bitrate %1").arg(argv[i])); }
      ...
      else if (mode == "j") { os.setStereoMode(OutputSettings::StereoMode_JointStereo); }
      else if (mode == "m") { os.setStereoMode(OutputSettings::StereoMode_Mono); }
      else { return usageError(QString("Invalid stereo mode %1").arg(argv[i])); }
      ...
      else if (ip == "sincmedium") { qs.interpolation = AudioEngine::qualitySettings::Interpolation_SincMedium; }
      else if (ip == "sincbest") { qs.interpolation = AudioEngine::qualitySettings::Interpolation_SincBest; }
  ...
  if (!renderOut.isEmpty()) {
    ...
    if (recoveryFilePresent) {
      ...
      mb.exec();
      if (mb.clickedButton() == discard) { getGUI()->mainWindow()->sessionCleanup(); } else if (mb.clickedButton() == recover)  {
    ...
    // a new one.
      ...
      if (recentFile.exists() && recentFile.suffix().toLower() != "mpt") { Engine::getSong()->loadProject(f); } else { Engine::getSong()->createNewProject(); }
    } else { Engine::getSong()->createNewProject(); }

    // Finally we start the auto save timer and also trigger the
    // autosave one time as recover.mmp is a signal to possible other
    // instances of LMMS.
    if (autoSaveEnabled) { gui::getGUI()->mainWindow()->autoSaveTimerReset(); }
  }
